...
class InviteTask(
    ...
    override fun run() {
        ...
        if (status == InviteStatus.ACCEPTED || status == InviteStatus.DENIED) {
            ...
            cancel()
            club.malvaceae.malloy.Malloy.log.info("[Invites] Invite removed because accepted/denied for ${invitee.name} to ${from.name} (${from.uniqueId}).")
        }
        if (timer == 0) {
            InviteHandler.removeInvite(this)
            cancel()
            club.malvaceae.malloy.Malloy.log.info("[Invites] Invite timed out for ${invitee.name} to ${from.name} (${from.uniqueId}).")
        }
    }
...
class InviteHandler {
    companion object {
        ...
        fun createInvite(from: Town, inviter: OfflinePlayer, invitee: OfflinePlayer) {
            ...
            club.malvaceae.malloy.Malloy.log.info("[Invites] Created invite for ${invitee.name} to ${from.name} (${from.uniqueId}).")
        }

        fun removeInvite(invite: InviteTask) {
            tasks[invite.invitee.uniqueId]!!.remove(invite)
            club.malvaceae.malloy.Malloy.log.info("[Invites] Removed invite for ${invite.invitee.name} from ${invite.from.name} (${invite.from.uniqueId}).")
        }
    }
}